...
//Greedy - We take the least weighed container and put it in the truck1 first in order to put maximum number of containers in truck.
//       We then do the same for truck 2
int truck :: cont_trk(int j,int x)
{
	int i=0,temp=x;
	while(temp>=arr[i] && i<=containers)  //If the particular container is <= the weight remaining to be filled in truck1 , we include it in the truck
	{
	    temp=temp-arr[i];
		i++;		
	}
	//for truck 2 again
	while(temp>=arr[i] && i<=containers)  //If the particular container is <= the weight remaining to be filled in truck2 , we include it in the truck
	{
	    temp=temp-arr[i];
		i++;		
	}
...
int main()
{
	...
	truck t1;
	cout<<"Enter the maximum capacity the trucks can hold: "; 
	cin>>t1.x;
	...
	//We first sort the elements based on weights to implement greedy => Time complexity = O(n^2) , where n = number of containers
	t1.merge_sort(0,t1.containers-1);  
	
     // We keep including the container with smallest weight to attain max number of containers   	
 	cout<<"MAX NUMBER OF CONTAINERS THAT CAN FIT IN TRUCK1 AND TRUCK2 = "<<t1.cont_trk(0,t1.x);  
		return 0;
}
